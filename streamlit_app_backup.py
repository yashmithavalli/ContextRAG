...
def init_ui():
    st.set_page_config(page_title="ContextRAG LLM", page_icon="🧠", layout="wide")
    st.title("ContextRAG 🧠")
    st.markdown("A Retrieval-Augmented Generation system powered by Google Gemini.")
            ...
            with st.spinner("Gemini is exploring context..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if not memory_disabled:
                            store_turn(SESSION_ID, prompt, answer, sources_list)
                    else:
